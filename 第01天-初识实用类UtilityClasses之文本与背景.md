...
# 教学大纲
没问题！为了让你心里有个底，我为你制定了一个**“循序渐进、每天一个小目标”**的 Tailwind 学习路线图。

我们的目标是：每天花 5-10 分钟，通过在 PlayCode 里改改代码，在不知不觉中掌握 Tailwind 的核心布局和样式技巧。

以下是接下来几天的教学大纲（你可以随时调整进度，想一天学两节也没问题）：

### 📍 第一阶段：基础样式（让元素好看）
*   **第 1 天（已完成）**：**文本与背景** —— 认识颜色色阶、字号和粗细（`text-blue-500`, `bg-gray-100`, `text-xl` 等）。
*   **第 2 天**：**呼吸感（间距）** —— 掌握内外边距 Padding & Margin（`p-4`, `px-2`, `mt-6` 等），这是网页排版的灵魂。
*   **第 3 天**：**尺寸控制** —— 调整宽度和高度（`w-full`, `h-64`, `w-1/2` 等），了解固定尺寸与百分比。
*   **第 4 天**：**边框与圆角** —— 给元素加上边框和现在最流行的圆角设计（`border`, `border-gray-200`, `rounded-xl`, `rounded-full` 等）。
*   **第 5 天**：**阴影与透明度** —— 增加卡片的立体感（`shadow-md`, `shadow-lg`, `opacity-75` 等）。

### 📍 第二阶段：核心布局（让元素排好队）
*   **第 6 天**：**Flexbox 弹性布局（上）** —— 告别乱跑的元素，让它们水平或垂直排列（`flex`, `flex-col`, `gap-4`）。
*   **第 7 天**：**Flexbox 弹性布局（下）** —— 完美居中与对齐（`justify-center`, `items-center`, `justify-between`）。
*   **第 8 天**：**Grid 网格布局** —— 轻松搞定照片墙、商品列表等多列等宽布局（`grid`, `grid-cols-3`）。
*   **第 9 天**：**定位系统** —— 绝对定位与相对定位，做右上角的小红点提示（`relative`, `absolute`, `top-0`, `right-0`）。

### 📍 第三阶段：交互与进阶（让网页活起来）
*   **第 10 天**：**状态修饰符（Hover & Focus）** —— 鼠标悬停变色、点击时的外圈高亮（`hover:bg-blue-600`, `focus:ring`）。
*   **第 11 天**：**响应式设计（神级功能）** —— 适配手机和电脑屏幕，只需加前缀（`md:flex`, `lg:w-1/3`）。
*   **第 12 天**：**过渡与动画** —— 让颜色的切换、元素的放大变得丝滑（`transition`, `duration-300`, `animate-spin`）。

---

**💡 这个路线图的设计思路：**
先从最简单的**“长什么样”**（颜色、大小、边框）开始，有了基本组件的样子后，再去学**“放在哪里”**（间距、Flex 布局），最后加上**“交互和响应式”**。

学完这些，你基本就可以用 Tailwind 随心所欲地画出 90% 以上的常见网页 UI 了。
